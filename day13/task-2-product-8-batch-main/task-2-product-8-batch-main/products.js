...
const btn = "All";
const ak = products.reduce((pre,value)=>{
...
ak.unshift(btn);
const Buttons = ak.map(function(value)
{
...
document.getElementById("companies").innerHTML = Buttons.join(" "); 
  ...
  function display(a)
   {
     let myProducts=a.map(function(value,index)
    {
        ...
        <img src="https://images.pexels.com/photos/1099680/pexels-photo-1099680.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1"
         class="product-img img"alt=""/>
          <footer>
             ...
             <button type="button" class="filter-btn" onclick="addToCart(${index})" data-id="all">add to cart</button>
         </footer>
         </article>`
     })
    document.getElementById("products").innerHTML = myProducts.join(" ");
  }
  ...
  { 
    ...
    {
      return value.company==company || company==btn; //condition for same category || when category button(All)
    })
  ...
  let addToCart = (index)=>
{ 
  let selectedproductsItems = products[index];
  let exit = carts.find((v)=>
  {
    return v.id == selectedproductsItems.id;
  });
  if(exit==undefined)
  {
    selectedproductsItems["qty"] =1;
    carts.push(selectedproductsItems);
  }
  else{
    selectedproductsItems["qty"] =  selectedproductsItems["qty"] + 1;
  }
  document.getElementById("Count").innerHTML = carts.length;
  document.getElementById("Quantity").innerHTML = selectedproductsItems["qty"];
}
